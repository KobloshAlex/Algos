package leetcode.custom.linkedlist.doublilinkedlist;
...
public class DoublyLinkedList {
  ...
  public Node deleteFirst() {
    ...
    first =
        first
            .next; // we are assinging the reference of the node following the old first node to the
    // first field in the linked list objec
  ...
  public boolean insertAfter(int key, int data) {
    ...
    if (current == last) {
      ...
      newNode.next =
          current.next; // new node's next field should point to the node ahead of the current one
      current.next.previous =
          newNode; // the node ahead of current, it's previous field should point to the new node
    }
  ...
  public Node deleteKey(int key) {
    ...
    if (current == first) {
      first =
          current
              .next; // make the first field point to the node following current since current will
      // be deleted
    } else {
